...
/**
 * Función para actualizar el total según el método de entrega
 */
function actualizarTotal() {
  const metodoDespacho = document.getElementById("metodoDespacho").checked;

  // Precios base (datos de ejemplo)
  const subtotal = 108970;
  const costoEnvio = metodoDespacho ? 8990 : 0;
  const totalConEnvio = subtotal + costoEnvio;
  const iva = Math.round(totalConEnvio * 0.19);
  const total = totalConEnvio + iva;
